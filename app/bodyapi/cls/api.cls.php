...
//        $sql['v']	= null;
    ...
    public function delUnitTest()
    {
        $data = array('basic', array(array("AND", "basictype = :basictype", 'basictype', 1)));
        $sql = $this->pdosql->makeDelete($data);
        $this->db->exec($sql);
    }

    public function getExamHistoryBuUserExam($args)
    {
        $data = array(false,'examhistory',$args);
        $sql = $this->pdosql->makeSelect($data);
        /*$sql['sql'] = "select * from x2_examhistory where ehbasicid=$examId and ehuserid=$userId";
        $sql['v']   = "";*/
        return $this->db->fetchAll($sql);
    }
}
