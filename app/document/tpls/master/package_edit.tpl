...
{x2;endif}
				...
				<div class="box itembox" style="padding-top:10px;margin-bottom:0px;">
					...
					<form action="index.php?document-master-attachtype-editPackage" method="post" class="form-horizontal">
						<fieldset>
							<div class="form-group">
								<label for="basic" class="control-label col-sm-2">套餐名称</label>
								<div class="col-sm-6">
									<input class="form-control" id="course_package_name" name="args[course_package_name]" type="text" value="{x2;$package[course_package_name]}" needle="needle" msg="您必须输入套餐名称" />
								</div>
							</div>

							<div class="form-group">
								<!--<label for="discern_id" class="control-label col-sm-2">鉴别听诊</label>
								<div class="col-sm-6">
									<input class="form-control" id="discern_name" type="text" value="{x2;$package[discern_name]}" />
									<input class="form-control" id="discern_id" name="args[discern_id]" style="display: none;" type="text" value="{x2;$package[discern_id]}" />
								</div>-->
								<label for="discern_id" class="control-label col-sm-2">听诊套餐</label>
								<div class="col-sm-6">
									<!--<input class="form-control" id="sound_package_name" type="text" value="{x2;$package[discern_name]}" />
									...
									<select name="args[discern_id]" class="form-control">
										...
										<option value="{x2;v:sp['soundcase_package_id']}" {x2;if:v:sp['soundcase_package_id']==$package['discern_id']}selected{x2;endif}>{x2;v:sp['package_name']}</option>
                                        {x2;endtree}
									</select>
								</div>
							</div>
								...
								<div class="row">
                                    ...
                                    <div class="col-xs-5">
                                        <select name="to[]" id="undo_redo_to" class="form-control" size="13" needle="needle" msg="您必须鉴别听诊或标准化病例" multiple="multiple" style="height:275px;">
                                            {x2;tree:$selectCourse,course,cid}
											<option value="{x2;v:course['system_course_id']}">{x2;v:course['course_name']}</option>
                                            {x2;endtree}
										</select>
                                    </div>
                                </div>
						...
						<div class="form-group">
							...
							<div class="col-sm-9">
								<textarea class="form-control" rows="4" name="args[memo]" id="memo" autocomplete="off">{x2;$package[memo]}</textarea>
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-sm-9">
								...
								<input type="hidden" name="course_package_id" value="{x2;$package[course_package_id]}"/>
                    ...
                    $.ajax({
                        ...
                        success:function (data) {
                            ...
                            $.each(clone, function(i, v) {
                                opt+="<option value='"+v['system_course_id']+"'>"+v['course_name']+"</option>";
                            });
                            $('#undo_redo').html(opt);
                        }
                    });
