...
class exam_exam
{
     ...
     * 添加操作试题
     * @param $args 表单数据
     * @return mixed 添加的主键ID
     */
    public function addOptQuestion($args)
    {
        $data = array('opt_question', $args);
        $sql = $this->pdosql->makeInsert($data);
        $this->db->exec($sql);
        return $this->db->lastInsertId();
    }
     ...
     * @param $args 表单数据
     */
    public function addOptQuestionItem($args)
    {
        $sql['sql'] = "DELETE FROM x2_opt_question_item WHERE opt_question_id=" . $args[0]['opt_question_id'];
        $sql['v'] = null;
        $this->db->exec($sql);

        foreach ($args as $v) {
            $data = array('opt_question_item', $v);
            $sql = $this->pdosql->makeInsert($data);
            $this->db->exec($sql);
        }
    }
     ...
     * 获取操作试题分页列表
     * @param $page 页码
     * @param int $number 每页显示条数
     * @param int $args 搜索条件
     * @return 操作试题LIST
     */
    public function getHeartLungQuestion($page, $number = 20, $args = 1)
    {
        $page = $page > 0 ? $page : 1;
        $r = array();
        $data = array(false, 'opt_question', $args, false, 'opt_question_id DESC', array(intval($page - 1) * $number, $number));
        $sql = $this->pdosql->makeSelect($data);
        $r['data'] = $this->db->fetchALL($sql);
        foreach ($r['data'] as $k => $v) {
            $sql["sql"] = "SELECT c.sound_file,c.case_name,oqi.* FROM x2_opt_question_item AS oqi
                    LEFT JOIN x2_case1 AS c ON oqi.case_id = c.case_id
                    WHERE oqi.opt_question_id=" . $v['opt_question_id']." ORDER BY oqi.item_title ASC";
            $sql["v"] = null;
            $r['data'][$k]['item'] = $this->db->fetchALL($sql);
            foreach ($r['data'][$k]['item'] as $key=>$value){
                if($value['case_id']==0) $r['data'][$k]['item'][$key]['case_name']=$value['item_content'];
            }
        }
        $data = array('COUNT(*) AS number', 'opt_question', $args, false, false, false);
        $sql = $this->pdosql->makeSelect($data);
        $tmp = $this->db->fetch($sql);
        $r['number'] = $tmp['number'];
     ...
     * @return 操作试题LIST
     */
    public function getOptionList($page, $number = 20, $args = 1)
    {
        $page = $page > 0 ? $page : 1;
        $r = array();
        $data = array(false, 'opt_question', $args, false, 'opt_question_id DESC', array(intval($page - 1) * $number, $number));
    ...
    public function getQuestionsByIds($questionids){
        $sql['sql'] = "SELECT * FROM x2_opt_question WHERE opt_question_id IN ($questionids)";
        $sql['v']   = null;
        ...
        foreach ($questions as $k => $v) {
            $sql["sql"] = "SELECT c.sound_file,c.case_name,oqi.* FROM x2_opt_question_item AS oqi
                    ...
                    WHERE oqi.opt_question_id=" . $v['opt_question_id']." ORDER BY oqi.item_title ASC";
            $sql["v"] = null;
            $questions[$k]['item'] = $this->db->fetchALL($sql);
            foreach ($questions[$k]['item'] as $key=>$value){
                if($value['case_id']==0) $questions[$k]['item'][$key]['case_name']=$value['item_content'];
            }
        }
        return $questions;
    }
     ...
     * @return 操作试题对象
     */
    public function getOptQuestionById($questionId)
    {
        $data = array(false, array('opt_question'), array(array('AND', "opt_question_id = :opt_question_id", 'opt_question_id', $questionId)));
        $sql = $this->pdosql->makeSelect($data);
        return $this->db->fetch($sql);
    }

    /**
     * 根据操作试题ID 获取试题与病例声音关系
     * @param $questionId 操作试题ID
     * @return 试题与病例关系LIST
     */
    public function getItemsByQuestionId($questionId)
    {
        $data = array(false, array('opt_question_item'), array(array('AND', "opt_question_id = :opt_question_id", 'opt_question_id', $questionId)), false,'item_title ASC');
        $sql = $this->pdosql->makeSelect($data);
     ...
     * 更新操作试题
     * @param $args 表单数据
     * @param $questionId 要更新的操作试题ID
     */
    public function updateOptQuestion($args,$questionId){
        $data = array('opt_question',$args,array(array('AND',"opt_question_id = :opt_question_id",'opt_question_id',$questionId)));
        $sql = $this->pdosql->makeUpdate($data);
        $this->db->exec($sql);
    }
}
