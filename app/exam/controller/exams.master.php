...
class action extends app
{
    ...
    private function optPage(){
        $this->tpl->display('exams_opt');
    }

    private function modifyOpt(){

        $examId = $this->ev->get('examid');
        $exam = $this->exam->getExamSettingById($examId);
        $exam['examnumber'][1] = $this->exam->getExamQuestionNumber($exam['examquestions'][1]);
        $this->tpl->assign('exam', $exam);
        $this->tpl->display('exams_opt_edit');
    }

    private function selectOptQuestions(){
        $search = $this->ev->get('search');
        $useframe = $this->ev->get('useframe');
        $page = $this->ev->get('page');
        $page = $page > 0 ? $page : 1;
        $this->pg->setUrlTarget('modal-body" class="ajax');
        if (strlen($search['organ_type'])>0){
            if($search['organ_type']==1){
                $args[] = array("AND", "organ_type = 0", 'organ_type', 0);
                $args[] = array("OR", "organ_type = 1", 'organ_type', 1);
            }else{
                $args[] = array("AND", "organ_type = :organ_type", 'organ_type', $search['organ_type']);
            }
        }

        $questions = $this->exam->getOptionList($page, 10, $args);
        ...
        $this->tpl->assign('options', $questions);
        $this->tpl->assign('useframe', $useframe);
        $this->tpl->display('select_opt_questions');
    }
    ...
    private function selectexams()
    {
        ...
        $basictype = $this->ev->get('basictype');
        $page = $this->ev->get('page');
        $page = $page > 0 ? $page : 1;
        $this->pg->setUrlTarget('modal-body" class="ajax');
        $args = array();
        ...
        if($basictype==1 || $basictype==0){
            $args[]=array("AND","examtype = :examtype","examtype",2);
        }
    ...
    private function selectOptExams()
    {
        $useframe = $this->ev->get('useframe');
        $target = $this->ev->get('target');
        ...
        $args[] = array("AND", "organ_type = :organ_type", 'organ_type', $organ_type);
        if (!count($args)) $args = 1;
        $exams = $this->exam->getExamSettingList($page, 10, $args);
        $subjects = $this->basic->getSubjectList();
        $this->tpl->assign('subjects', $subjects);
        $this->tpl->assign('target', $target);
        $this->tpl->assign('exams', $exams);
        $this->tpl->display('opt_exams_ajax');
    }
    ...
    private function previewOpt(){
        ...
        if($r['question_type']==1){
            foreach ($r['examquestions'] as $key => $p) {
                ...
                if ($qids)
                    $questions[$key] = $this->exam->getQuestionsByIds($qids);
        ...
        $this->tpl->display('exams_paper_opt');
    }
            ...
            if($r['question_type']==1){
                ...
                foreach ($question as $k=>$v){
                    foreach ($v as $k1=>$v1){
                       ...
                       $questions[$k][$k1]['questiontype'] = $v1['organ_type']==3? 1:2;
                       ...
                       $questions[$k][$k1]['questionlevel'] = 1;
                        ...
                        foreach ($v1['item'] as $kk=>$vv){
                            $selectStr.=$vv['item_title'].":".$vv["item_content"]."&lt;/p&gt;&lt;p&gt;";
                        }
                        $questions[$k][$k1]['questionselect'] = $selectStr;
                    }
                }
            }
    ...
    private function selectedOpt()
    {
        $show = $this->ev->get('show');
        $questionids = trim($this->ev->get('questionids'), " ,");
        $questions=$this->exam->getQuestionsByIds($questionids);
        $this->tpl->assign('questions', $questions);
        $this->tpl->assign('show', $show);
        $this->tpl->display('exams_selected_opt');
    }
    ...
    private function modify()
    {
        $search = $this->ev->get('search');
        $examid = $this->ev->get('examid');
        if(!$examid) $examid = $this->ev->post('examid');
        $exam = $this->exam->getExamSettingById($examid);
        if ($this->ev->get('submitsetting')) {
            $args = $this->ev->get('args');
            $args['examsetting'] = $args['examsetting'];
    ...
    private function index()
    {
        ...
        if ($search) {
            ...
            if (strlen($search['question_type'])>0) $args[] = array("AND", "question_type = :question_type", 'question_type', $search['question_type']);
            if (strlen($search['organ_type'])>0) {
                if($search['organ_type']==5){
