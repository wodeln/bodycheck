...
class action extends app
{
	...
	private function optionQuestion(){
        ...
        $options = $this->exam->getOptionList($page,10,$args);
        $this->tpl->assign('options',$options);
        $this->tpl->assign('search',$search);
        $this->tpl->assign('u',$u);
        $this->tpl->assign('page',$page);

        $this->tpl->display('opt_questions');
	}

	private function heartLungQuestion(){
        $page = $this->ev->get('page')?$this->ev->get('page'):1;
        $search = $this->ev->get('search');
        $u = '';
        if($search)
        ...
        if(strlen($search['organ_type']))$args[] = array('AND',"organ_type = :organ_type",'organ_type',$search['organ_type']-1);
        else $args[] = array('AND',"organ_type IN (0,1)",'organ_type',"0,1");
        $options = $this->exam->getHeartLungQuestion($page,10,$args);
        $this->tpl->assign('options',$options);
        $this->tpl->assign('search',$search);
        $this->tpl->assign('u',$u);
        $this->tpl->assign('page',$page);

        $this->tpl->display('ht_questions');
	}

    private function cardiogramQuestion(){
        $page = $this->ev->get('page')?$this->ev->get('page'):1;
        $search = $this->ev->get('search');
        $u = '';
        if($search)
        ...
        $args[] = array('AND',"organ_type =:organ_type",'organ_type',"4");
        $options = $this->exam->getHeartLungQuestion($page,10,$args);
        $this->tpl->assign('options',$options);
        $this->tpl->assign('search',$search);
        $this->tpl->assign('u',$u);
        ...
        $this->tpl->display('ht_cardiogram_questions');
    }

    private function touchQuestion(){
        $page = $this->ev->get('page')?$this->ev->get('page'):1;
        $search = $this->ev->get('search');
        $u = '';
        if($search)
        ...
        $args[] = array('AND',"organ_type =:organ_type",'organ_type',"2");
        $options = $this->exam->getHeartLungQuestion($page,10,$args);
        $this->tpl->assign('options',$options);
        $this->tpl->assign('search',$search);
        $this->tpl->assign('u',$u);
        ...
        $this->tpl->display('ht_touch_questions');
    }

	private function addOptQuestionCardiogram(){
        $this->tpl->display('opt_question_cardiogram_add');
	}

	private function addOptQuestion(){
		$page = $this->ev->get("page");
		if($this->ev->post("insertOptQuestion")){
			...
			$item = "";
			if($args['organ_type']==4){
                foreach(range('A','D') as $k=>$v){
                    $item[$k]['opt_question_id'] = $optId;
                    $item[$k]['item_title'] = $v;
                    $item[$k]['case_id'] = $this->ev->post($v);
                ...
                foreach(range('A','E') as $k=>$v){
                    ...
                    $item[$k]['item_title'] = $v;
                    $item[$k]['case_id'] = $this->ev->post($v);
                    $item[$k]['item_content'] = $this->ev->post("t_".$v);
                    if($args['right_item'] == $v) $item[$k]["if_right"] = 1;
                    else $item[$k]["if_right"] = 0;
                }
			}
            ...
            $message = array(
                ...
                "message" => "操作成功",
                "callbackType" => "forward",
                "forwardUrl" => "index.php?exam-master-questions-optionQuestion&page={$page}{$this->u}"
            );
            exit(json_encode($message));
		}else{
            $this->tpl->assign('page',$page);
            $this->tpl->display('opt_question_add');
		}

	}

	private function modifyOptQuestion(){
		if($this->ev->post('modifyOptQuestion')){
            $args = $this->ev->post("args");
            /*$user = $this->user->getUserById($this->session->getSessionUser()['sessionuserid']);
            $args['add_name']= $user['usertruename'];
            $args['add_time'] = time();*/
            $questionId = $this->ev->post("questionId");
            $page = $this->ev->post("page");
            $this->exam->updateOptQuestion($args,$questionId);
            $item = "";
            if($args['organ_type']==4){
                foreach(range('A','D') as $k=>$v){
            ...
            $type = $this->ev->get('organ_type');
            if($type==4){
                $templet = "opt_question_cardiogram_edit";
            }else{
                $templet = "opt_question_edit";
            }
            $page = $this->ev->get("page");
            $questionId = $this->ev->get("questionid");
            $optQuestion  = $this->exam->getOptQuestionById($questionId);
            $items = $this->exam->getItemsByQuestionId($questionId);
            $this->tpl->assign("optQuestion",$optQuestion);
            $this->tpl->assign("items",$items);
            $this->tpl->assign("page",$page);
            $this->tpl->display($templet);
		}
    }
}
