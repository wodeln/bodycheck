			        ...
			        <form action="index.php?exam-master-exams" method="post" class="form-inline">
						<table class="table">
							...
							<tr>
								<td>
									试卷分类：
									<select name="search[organ_type]" id="question_type" class="form-control">
										<option value="">不限</option>
										<option {x2;if:$search['organ_type'] == 5} selected{x2;endif} value="5">理论试卷</option>
										<option {x2;if:$search['organ_type'] == 1} selected{x2;endif} value="1">心肺音</option>
										<option {x2;if:$search['organ_type'] == 2} selected{x2;endif} value="2">腹部触诊</option>
										<option {x2;if:$search['organ_type'] == 4} selected{x2;endif} value="4">心电图</option>
									</select>
								</td>
								<td id="organ_type_td">
									<!--实际操作分类：
									<select name="search[organ_type]" id="organ_type" class="form-control">
										<option value="">不限</option>
										<option {x2;if:$search['organ_type'] == 1} selected{x2;endif} value="1">心肺音</option>
										<option {x2;if:$search['organ_type'] == 2} selected{x2;endif} value="2">肠鸣音</option>
										<option {x2;if:$search['organ_type'] == 4} selected{x2;endif} value="4">心电图</option>
									</select>-->
								</td>
								<td></td>
								...
								<td>
									...
									<input type="hidden" value="1" name="search[argsmodel]" /></td>
							</tr>
						</table>
					</form>
					        ...
					        <tr>
								...
								<td>
									<div class="btn-group">
			                    		...
			                    		{x2;endif}
										{x2;if:v:exam['question_type']==2}
										<a class="btn" href="index.php?{x2;$_app}-master-exams-modifyOpt&page={x2;$page}&examid={x2;v:exam['examid']}{x2;$u}" title="修改"><em class="glyphicon glyphicon-edit"></em></a>
										<a class="btn confirm" href="index.php?{x2;$_app}-master-exams-delOpt&page={x2;$page}&examid={x2;v:exam['examid']}{x2;$u}" title="删除"><em class="glyphicon glyphicon-remove"></em></a>
										{x2;endif}
                                        {x2;if:v:exam['question_type']==1}
...
<script type="text/javascript">
    ...
    $("#question_type").change(function () {
        if($(this).val()==2){
            $("#organ_type_td").css("display","block");
        }else {
            $("#organ_type_td").css("display","none");
            $("#organ_type").val("");
        }
    });
</script>
